...
export default function CheckInPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const session = await apiFetch<{ id: string }>("/parking-sessions", {
        method: "POST",
        body: JSON.stringify({
      ...
      <div className="px-4 sm:px-6 py-8 max-w-screen-xl mx-auto flex items-center justify-center min-h-[70vh]">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-500 mb-2">
            Truck <span className="font-semibold text-gray-800">{truckNumber}</span> has been checked in.
          </p>
          <p className="text-sm text-gray-400 mb-1">Session ID</p>
          <p className="font-mono text-xs bg-gray-100 text-gray-700 rounded-lg px-4 py-2 mb-8 break-all">{sessionData.id}</p>
          <div className="flex gap-3 justify-center">
            <button
              onClick={() => {
                setSessionData(null);
                setTruckNumber(""); setTruckId(null); setTruckFound(null); setTruckType("");
                setOwnerId(null); setOwnerName(""); setOwnerMobile(""); setOwnerCompany(""); setOwnerAlt("");
                setDriverName(""); setDriverMobile(""); setDriverLic(""); setRemarks("");
                setSlotId(""); setDivisionId(""); setCheckInDate(todayStr()); setCheckInTime(nowTimeStr());
